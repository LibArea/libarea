...
<main class="col-span-7 mb-col-12">
  <div class="bg-white br-rd5 br-box-grey mb15 p15">
    <div class="flex flex-row items-center justify-between">
      <div class="no-mob">
        <?= topic_logo_img($topic['topic_img'], 'max', $topic['topic_title'], 'w94 br-box-grey mt5'); ?>
      </div>
        ...
        <div class="mt15 right">
          <?php if (!$uid['user_id']) { ?>
            <a href="<?= getUrlByName('login'); ?>">
              <div class="bg-gray-200 bg-hover-gray mazarine br-box-grey inline br-rd20 center pt5 pr15 pb5 pl15">+ <?= Translate::get('read'); ?></div>
            </a>
          <?php } else { ?>
            <?php if (is_array($data['topic_signed'])) { ?>
              <div data-id="<?= $topic['topic_id']; ?>" data-type="topic" class="focus-id bg-gray-100 gray-light-2 br-box-grey inline br-rd20 center pt5 pr15 pb5 pl15">
                <?= Translate::get('unsubscribe'); ?>
              </div>
            <?php } else { ?>
              <div data-id="<?= $topic['topic_id']; ?>" data-type="topic" class="focus-id bg-gray-200 bg-hover-gray mazarine br-box-grey inline br-rd20 center pt5 pr15 pb5 pl15">
                + <?= Translate::get('read'); ?>
              </div>
            <?php } ?>
          <?php } ?>
          ...
          <div class="size-14 mt20 gray-light-2">
            ...
            foreach ($data['focus_users'] as $user) {
              ...
              <a class="-mr-1 bg-white" href="<?= getUrlByName('user', ['login' => $user['user_login']]); ?>">
                <?= user_avatar_img($user['user_avatar'], 'max', $user['user_login'], 'w24 br-rd-50 br-box-grey'); ?>
              </a>
            <?php } ?>
...
<aside class="col-span-3 relative no-mob">
  <div class="bg-white br-rd5 mb15 br-box-grey p15">
  ...
  <?php if (!empty($data['writers'])) { ?>
    <div class="sticky top0 top80">
      <div class="br-box-grey mt15 p15 mb15 br-rd5 bg-white size-14">
