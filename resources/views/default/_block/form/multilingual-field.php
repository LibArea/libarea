<fieldset class="tabs" id="<?= $name; ?>">
  <div class="tabs__nav">
    <?php foreach (config('general.languages') as $key => $lang) : ?>
      <button class="tabs__btn<?php if (config('general.lang') == $lang) { ?> tabs__btn_active<?php } ?>"><?= $lang; ?></button>
    <?php endforeach; ?>
  </div>
  <div class="tabs__content">
    <?php foreach (config('general.languages') as $key => $lang) : ?>
      <div class="tabs__pane<?php if (config('general.lang') == $lang) { ?> tabs__pane_show<?php } ?>">
        <label for="<?= $name; ?>"><?= __('app.' . $name); ?> <span class="help">(<?= __('app.' . $lang . '_language'); ?>)</span> <sup class="red">*</sup></label>
