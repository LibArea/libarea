  ...
  <main class="w-100 white-box pt5 pr15 pb5 pl15 edit-post">
    ...
    <form action="/post/edit" method="post" enctype="multipart/form-data">
      ...
      <?php if ($uid['user_trust_level'] > 0) { ?>
        <?php select('trust_level', ['uid' => $uid, 'data' => $data['post']['post_tl']]); ?>
        <?php field('radio', [
      ...
      <?php if ($uid['user_trust_level'] > 4) { ?>
        <?php select('select', ['type' => 'user', 'data' => $data, 'action' => 'edit', 'title' => lang('Author')]); ?>
      <?php } ?>

      <?php select('select', ['type' => 'topic', 'data' => $data, 'action' => 'edit', 'title' => lang('Topics')]); ?>
      <?php select('select', ['type' => 'post', 'data' => $data, 'action' => 'edit', 'title' => lang('Related')]); ?>
