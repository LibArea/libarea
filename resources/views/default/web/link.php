      ...
      <?php if ($data['link']['link_title']) { ?>
        ...
        <h1><?= $data['link']['link_title']; ?>
          <?php if ($uid['user_trust_level'] > 4) { ?>
            <a class="size-13 ml5" title="<?= lang('Edit'); ?>" href="<?= getUrlByName('link-edit', ['id' => $data['link']['link_id']]); ?>">
    ...
    <div class="white-box pt5 pr15 pb5 pl15 space-tags">
      <?php if (!empty($data['domains'])) { ?>
        ...
        <?php foreach ($data['domains'] as  $domain) { ?>
          <a class="size-13 gray" href="<?= getUrlByName('domain', ['domain' => $domain['link_url_domain']]); ?>">
