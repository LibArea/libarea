<?php
$data = [
    'home'              => '家',
    'to.the.website'    => '到网站',
    'catalog'           => '目錄',
    'feed'              => '饲料',
    'hi'                => '你好',
    'news'              => '新闻',
    'team'              => '團隊',
    'teams'             => '團隊',
    'owner'             => '所有者',
    'top'               => '最佳',
    'admin'             => '管理面板',
    'well'              => '好吧',
    'sort'              => '排序',
    'facets'            => '刻面',
    'content'           => '內容',
    'navigation'        => '導航',
    'category'          => '类别',
    'visibility'        => '能見度',
    'number'            => '數字',
    'icon'              => '圖標',
    'template'          => '模板',
    'settings'          => '设置',
    'security'          => '安全',
    'avatar'            => '头像照片',
    'understands'       => '了解',
    'necessarily'       => '需要填写',
    'create'            => '创建',
    'writers'           => '作家',
    'by.deposit'        => '按金支付',
    'welcome'           => '欢迎',
    'create.account'    => '创建账户',
    'not.registered'    => '发帖、回复、评论... <br>您还没有注册',
    'saw'               => '锯',
    'the.post'          => '帖子',
    'bodies'            => '身体',
    'thanks'            => '谢谢',
    'whisper'           => '耳语',
    'report'            => '报告',
    'reports'           => '報告',
    'content.deleted'   => '{name} 已刪除',
    'content.added'     => '{name} 添加',
    'content.restored'  => '{name} 已恢復',
    'time'              => '時間',
    'back'              => '後退',
    'edit'              => '改變',
    'save'              => '保存',
    'add'               => '添加',
    'add.option'        => '添加 ({name})',
    'edit.option'       => '改變 ({name})',
    'change.saved'      => '更改已保存',
    'successfully'      => '成功地！',
    'desc.formed'       => '描述形成',
    'being.developed'   => '正在開發中',
    'activity'          => '活動',
    'question.closed'   => '問題已關閉',
    'this.draft'        => '這篇文章是草稿',
    'no.cover'          => '還沒有封面',
    'no.invites'        => '您尚未創建邀請',
    'no.content'        => '抱歉，暫無內容',
    'no.dialogs'        => '抱歉，還沒有對話',    
    'no.posts'          => '抱歉，暫時沒有帖子',
    'no.favorites'      => '抱歉，還沒有書籤',
    'no.comments'       => '不幸的是，還沒有評論',
    'no.answers'        => '抱歉，暫時沒有答案',
    'default'           => '默認',
    'qa'                => '問題與解答',
    'answered'          => '已回答',
    'translation'       => '翻譯',
    'format.Q&A'        => '問答形式',
    'pin'               => '別針',
    
    'deleted'           => '已刪除',
    'remote'            => '偏僻的',
    'email.activated'   => '電子郵件已激活',
    'not.activated'     => '未激活',
    'like'              => '喜歡',
    'all.comments'      => '所有評論',
    'all.answers'       => '所有答案',
    'recommended'       => '受到推崇的',
    'notification'      => '通知',
    'notifications'     => '通知',
    'preferences'       => '喜好',
    'action'            => '行動',
    'domain'            => '領域',
    'domains'           => '域',
    'posted'            => '已發布',
    'blog'              => '博客',
    'blogs'             => '博客',
    'topics'            => '話題',
    'topic'             => '話題',
    'parent'            => '家長',
    'authorization'     => '授權',
    'sign.in'           => '登入',
    'sign.out'          => '登出',
    'registration'      => '註冊',
    'advertising'       => '廣告',
    'private.messages'  => '私人信息',
    'it.was.read'       => '已讀',
    'all.messages'      => '所有消息',
    'send.message'      => '發送一個消息',
    'dialogues'         => '對話',
    'dialogue'          => '對話',
    'write.message'     => '寫一個信息',
    'send'              => '發送',
    'information'       => '信息',
    'review'            => '審查',
    'contacts'          => '聯繫人',
    'city'              => '城市',
    'remember.me'       => '記得我',
    'banned'            => '禁止',
    'ban.it'            => '禁止它',
    'unban'             => '解禁',
    'dumb.mode'         => '啞模式',
    'reset'             => '重啟',
    'to create'         => '去創造',
    'invited.guests'    => '特邀嘉賓',
    'nickname'          => '暱稱',
    'name'              => '名稱',
    'password'          => '密碼',
    'new.password'      => '新密碼',
    'repeat'            => '重複',
    'show'              => '展示',
    'new'               => '新的',
    'new.ones'          => '新的',
    'old'               => '老的',
    'code'              => '代碼',
    'code.error'        => '代碼錯誤',
    'invite'            => '邀請',
    'invites'           => '邀請',
    'view'              => '觀看',
    'views'             => '觀看次數',
    'profile'           => '輪廓',
    'title'             => '標題',
    'heading'           => '標題',
    'to.extract'        => '提取',
    'discussion'        => '討論',
    'description'       => '描述',
    'meta.description'  => '元描述',
    'short.description' => '簡短的介紹',
    'favorites'         => '收藏夾 ',
    'long'              => '長',
    'created.by'        => '由...製作',
    'help'              => '幫助',
    'statistics'        => '統計數據',
    'privacy'           => '隱私',
    'about'             => '關於',
    'about.me'          => '關於我',
    'riddle'            => '謎語',
    'info'              => '信息',
    'more'              => '更多的',
    'other'             => '其他',
    'official'          => '官方的',
    'community'         => '社區',
    'trust.level'       => '信任級別',
    'search'            => '搜索',
    'to.find'           => '尋找',
    'up'                => '向上',
    'user'              => '用戶',
    'users'             => '用戶',
    'page'              => '頁',
    'read'              => '讀',
    'reads'             => '讀取',
    'reading'           => '我讀',
    'is.reading'        => '在看書',
    'subscribed'        => '已訂閱',
    'unsubscribed'      => '退訂',
    'unsubscribe'       => '退訂',
    'subscribe'         => '訂閱',
    'to.approve'        => '批准',
    'approved'          => '得到正式認可的',
    'off.mute.mode'     => '並關閉與會者的靜音模式',
    'remove'            => '消除',
    'recover'           => '恢復',
    'download'          => '下載',
    'error in loading'  => '加載錯誤',
    'password.recovery' => '找回密碼',
    'draft'             => '草稿',
    'drafts'            => '草稿',
    'cancel'            => '取消',
    'reply'             => '回复',
    'reply.web'         => '回答（到目錄）',
    
    // 編輯
    'foto'              => '一張照片',
    'bold'              => '脂肪',
    'strikethrough'     => '刪除線',
    'italic'            => '斜體',
    'quote'             => '引用',
    'link'              => '關聯',
    'preview'           => '預習',
    'toggle.two'        => '兩個窗口',
    'fullscreen'        => '全屏',
    'spoiler'           => '擾流板',
    'spoiler.auth'      => '僅用於授權',
    'select.image'      => '請選擇一張圖片',
    'drop.image'        => '拖動圖片上傳.',
    'attach.files'      => '文件可以從剪貼板粘貼.',
    
    'source'            => '來源',
    'website'           => '網站',
    'websites'          => '网站',
    'sites'             => '网站',
    'related'           => '有關的',
    'related posts'     => '相關文章',
    'related topics'    => '相關話題',
    'see more'          => '查看更多',
    'more.detailed'     => '更多的',
    'parents'           => '父母',
    'children'          => '孩子們',
    'bound.children'    => '綁定（兒童）',
    'bound.parents'     => '綁定（父母）',
    'root'              => '根',
    'root.help'         => '樹的頂部（主要）主題。',
    'upper'             => '上',
    'subtopics'         => '副題',
    'badge'             => '徽章',
    'badges'            => '徽章',
    'reward.user'       => '獎勵用戶',
    'for.example'       => '例如',
    'ed'                => '編輯',
    'add'               => '添加',
    'there.are'         => '有',
    'yes'               => '是的',
    'no'                => '不',
    'all'               => '全部',
    'alls'              => '整个',
    'you'               => '你',
    'from'              => '從',
    'by'                => '經過',
    'in'                => '在',
    'and'               => '和',
    'on'                => '在',
    'for'               => '為了',
    'my'                => '我的',
    'by.default'        => '默認情況下',
    'publications'      => '出版物',
    'selected.post'     => '選擇的帖子',
    'color'             => '顏色',
    'text'              => '文本',
    'author'            => '作者',
    'characters'        => '人物',
    'last.answer'       => '最後回答',
    'no.auth.comm'      => '您必須登錄後才能發表評論',
    'supports.markdown' => '支持降價',
    'write'             => '寫',
    'in.profile'        => '在個人資料中',
    'request'           => '要求',
    'results'           => '結果',

    'language'          => '本土化',
    'ru.language'       => '俄語 (RU)',
    'en.language'       => '英語 (EN)',
    'de.language'       => '德語 (DE)',
    'fr.language'       => '法語 (FR) ',
    'zh.language'       => '中文 (ZH)',
    'ro.language'       => '罗马尼亚语 (RO)',

    'close?'            => '關閉?',
    'repeat.password'   => '重複輸入密碼',
    'post.closed'       => '帖子已關閉',
    'forgot.password'   => '忘記密碼了嗎',
    'rules'             => '網站規則',
    'restriction'       => '限制',
    'invate.text'       => '還不是用戶？ 邀請註冊僅用於打擊垃圾郵件和增加問責制。 如果您知道該網站的<a href="/users">當前用戶</a>，請向他發出邀請。',
    
    'text.closed'       => '未經授權禁止查看此隱藏文字',

    /* 日期和時間 + 事件 */
    'months'        => ['', '一月', '二月', '瑪莎', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'],
    'today'         => '今天',
    'yesterday'     => '昨天',

    'num.message'   => ['消息', '消息', '消息'],
    'num.comment'   => ['評論', '評論', '評論'],
    'num.answer'    => ['答案', '答案', '答案'],
    'num.post'      => ['帖子', '帖子', '帖子'],
    'num.view'      => ['視圖', '視圖', '視圖'],
    'num.website'   => ['網站', '站點', '站點'],
    'num.blog'      => ['博客', '博客', '博客'],
    'num.up'        => ['声音', '声音', '声音'],

    'comment'       => '評論',
    'comments'      => '評論',
    'answer'        => '回答',
    'answers'       => '答案',
    'message'       => '消息',
    'messages'      => '消息',
    'post'          => '發布',
    'posts'         => '帖子',

    'type'                  => '類型',
    'logo'                  => '標識',
    'last'                  => '最後的',
    'cover.removed'         => '蓋子已移除',
    'recommended.size'      => '推薦尺寸',

    /* 通知 */
    'i.read'                    => '我讀',
    'successfully.login'        => '成功，登錄',
    'notif.add.pm'              => '寫信給你 {url} 私人信息 {a}',
    'notif.add.comment'         => '回复 {url} 你的答案 {a}',
    'notif.add.comment.comment' => '回复 {url} 你的評論 {a}',
    'notif.add.answer'          => '回复 {url} 你的帖子 {a}',
    'notif.add.flag'            => '註冊 {url} 抱怨 {a}',
    'notif.add.audit'           => '需要審計 {url} 內容  {a}',
    'notif.references.post'     => '{url} 在帖子中提到了你 {a}',
    'notif.references.amswer'   => '{url} 在回復中提到了你 {a}',
    'notif.references.comment'  => '{url} 在留言裡提到了你 {a}',
    'notif.add.website'         => '添加網站, {url} 看 {a}',
    'notif.edit.website'        => '更改站點, {url} 看 {a}',
    'notif.approved.website'    => '（主持人） {url} 更改（批准）您的網站 {a}',
    'notif.add.website.comment' => '{url} 评论了您订阅的网站 {a}',

    /* MSG */
    'string.length'         => '長度 {name} 應該來自 {min} -> {max} 人物 ',
    'slug.correctness'      => '{name} 應僅包含拉丁字母、數字',
    'titles'                => '標題',
    'descriptions'          => '說明',
    'repeat.url'            => '此主題網址已存在',
    'url.gaps'              => 'URL 中不允許使用博客',
    'select.topic'          => '選擇一個主題',
    'you.answered'          => '你已經回答了這個問題',
    'need.to.login'         => '需要登錄',
    'no.auth.login'         => '對於答案，您可以 <a href="/login">登錄</a>',
    'no.user'               => '成員不存在',

    'invite.created'        => '邀請已創建',
    'invalid'               => '無效的',
    'password.changed'      => '密碼更換成功',
    'old.password.err'      => '舊密碼不正確',
    'pass.match.err'        => '密碼有誤',
    'login.use.condition'   => '繼續，即表示您同意<a href="/info/article/privacy">使用條款</a>網站',
    'user.already'          => '該用戶已經在該網站上',
    'invate.to.replay'      => '您已經向該用戶發送了邀請',
    'invate.limit.stop'     => '您已用盡邀請數量',
    'limit.day'             => '為了 {tl} 你已經達到極限 «{name}» 今天',
    'limit.add.content.no'  => '您已達到站點添加集的上限',
    'registration.invite'   => '邀請註冊',
    'details.here'          => '詳情在這裡',
    'breaking.rules'        => '打破規則',
    'five.hundred.width'    => '封面寬度小於 500px',

    'sections'              => '節',
    'section'               => '節',
    'menu'                  => '菜單',
    'category'              => '類別',
    'categories'            => '類別',
    'categories.s'          => '類別',

    'tag'                   => '標籤',
    'tags'                  => '標籤',
    'tags.s'                => '標籤',
    'folder'                => '文件夾',
    'folders'               => '文件夾',
    'folders.s'             => '文件夾',

    'pages'                 => '頁面',
    'go.to'                 => '前往',
    'frameworks'            => '框架',
    'cms'                   => 'CMS',
    'seo'                   => 'SEO',
    'url'                   => 'URL',
    'slug'                  => 'SLUG',
    'interesting sites'     => '有趣的網站',
    'facts'                 => '事實',
    'psychology'            => '心理學',

    'edit.answer'           => '改變答案',
    'enter'                 => '進入',
    'enter.password'        => '輸入密碼',
    'enter.content'         => '輸入內容',

    /* 環境 */
    'setting.info'          => '填寫您的姓名，這將有助於與會者通過姓名來稱呼您。 <br> <br> 告訴我們一些關於你自己的事情。 你有一個很好的機會敞開心扉取悅他人，並且看起來是一個有趣的人。',
    'avatar.info'           => '為您的個人資料上傳頭像和封面。 <br> <br> 當每個用戶都有一個獨特的頭像時，關注討論會變得更容易！',
    'security.info'         => '確保您的所有密碼都是 8 個字符或更多，最好是更多。',
    'login.info'            => '授權使您能夠發布帖子、回复、評論。',
    'recover.info'          => '單擊“重置”按鈕後，檢查您的電子郵件...',
    'favorite.info'         => '書籤中的帖子、評論和問題',
    'preferences.info'      => '您訂閱的帖子。 您將收到所有新回复的通知',
    'endless.scroll'        => '啟用無限滾動？',
    'notification.email'    => '通過電子郵件通知',
    'notifications.info'    => '您可以通過單擊以下鏈接將所有廣告標記為已讀：“我已閱讀”。',
    'notification.info'     => '您可以在此部分取消訂閱通過電子郵件發送的所有通知',
    'notifications.limit'   => '最后100个通知显示',

    'message.PM'            => '當消息來到 PM',
    'appeal.@'              => '當他們通過@聯繫我時 ',
    'code.incorrect'        => '代碼不正確或已被使用。 再次執行恢復過程。',
    'yes.email.pass'        => '現在您可以使用您的電子郵件和密碼。',
    'code.used'             => '代碼不正確或已被使用。',
    'audits'                => '審計',
    'content.audit'         => '您的消息已發送以供審核。',
    'limit.tl.invitation'   => '您的信任級別還不允許使用邀請',
    'can.send.this.link'    => '你可以發送這個鏈接 ',
    'link.used'             => '鏈接已被使用',
    'medal.registration'    => '註冊獎牌',
    'registered'            => '掛號的 ',
    'invitations.left'      => '留下的邀請',

    'under.reconstruction'  => '該網站正在重建中',
    'silent.mode'           => '您無法發布資料，您的帳號正在驗證中',
    'no.dialogue'           => '指定的對話框不存在',
    'no.topic'              => '指定的主題不存在',
    'site.replay'           => '這樣的站點已經在系統中',
    'first.days'            => '在社區啟動的早期加入',
    'nickname.replay'               => '該暱稱已在網站上',
    'email.replay'                  => '此電子郵件已在網站上',
    'multiple.accounts'             => '不要註冊多個賬戶 ',
    'password.spaces'               => '密碼不能包含空格',
    'nick.character.repetitions'    => '暱稱重複字符過多',
    'code.incorrect'                => '代碼不正確或已被使用',
    'format.cover.post'             => '寬度大於 500px。 水平方向',
    'email.password.not.correct'    => '電子郵件或密碼不正確',
    'account.not.activated'         => '您的帳戶未激活',
    'account.being.verified'        => '您的帳戶待處理',
    'email.correctness'             => '不合規電郵',
    'email.no.site'                 => '該電子郵件不在網站上',
    'english'                       => '英語',
    'work.email'                    => '工作郵箱（用於<b>激活</b>帳戶）',
    'new.password.email'            => '重置密码的链接已发送到电子邮件',
    'check.your.email'              => '檢查您的電子郵件以激活您的帳戶',
    'view.post.tl'                  => '誰可以查看帖子（來自哪個 TL）',
    'breaking.rules'                => '這違反了網站的規則',
    'invited.you'                   => '您是受此網站成員邀請的。 輸入邀請',
    'invite.features'               => '你可以邀請朋友',
    'messages.info'             => '與網站成員的私信',
    'page.not'                  => '該頁面不存在',
    'to.main'                   => '主要',
    'page.removed'              => '該頁面已被刪除',
    'command.executed'          => '命令執行',

    'recommended posts'         => '推薦帖子',
    'recommended.posts.desc'    => '推薦的帖子 «{name}» 手選.',

    'topic.subscription'        => '訂閱主題並在提要中閱讀它們',
    'url.reserved'              => '此博客網址已保留。',
    
    /* 元描述  */
    'profile.posts.title'       => '{name} — 會員資料、帖子 ',
    'profile.posts.all.title'   => '{name} — 帖子列表 ',
    'profile.answers.title'     => '{name} — 參與者回复 ',
    'profile.comments.title'    => '{name} — 參與者評論 ',
    'profile.posts.desc'        => '{name} — 會員資料頁面。 帖子、獎項和聯繫方式. {information}',
    'profile.posts.all.desc'    => '{name} — 帖子列表，按時間順序排列的參與者的文章。 網站上的所有出版物.',
    'profile.answers.desc'      => '{name} — 參與者對主題和博客中的帖子的回复頁面. {information}',
    'profile.comments.desc'     => '{name} — 參與者評論按時間順序排列. {information}',

    'users.all'                 => '參賽者名單',
    'users.all.info'            => '參與者名單按他們在網站上的活動順序排列',
    'users.all.desc'            => '照他們在網站上的活動順序排列。',
    
    'users.new'                 => '新成員',
    'users.new.info'            => '最近註冊該網站的成員列表',
    'users.new.desc'            => '最近在網站上註冊的會員。 顯示活動、頭像、個人資料',
    
    'blogs.all'                 => '所有博客',
    'blogs.all.info'            => '貢獻者創建的博客',
    'blogs.all.desc'            => '博客按照他們在網站上的註冊順序，能夠查看創建的博客，訂閱它們，查看統計信息',

    'blogs.new'                 => '新博客',
    'blogs.new.info'            => '最近在網站上添加的博客',
    'blogs.new.desc'            => '最近添加了網站上的博客。 參與者的個人博客，能夠訂閱有趣的博客', 
    
    'blogs.my'                  => '我的博客',
    'blogs.my.info'             => '我關注或創建的博客',
    'blogs.my.desc'             => '我閱讀或創建的博客',
    
    'topics.all'                => '所有主題',
    'topics.all.info'           => '網站上的所有主題按受歡迎程度排序',
    'topics.all.desc'           => '按受歡迎程度排列的主題頁面。 通過定位的帖子數量，訂閱博客的能力',
    
    'topics.new'                => '新話題',    
    'topics.new.info'           => '新主題，最近添加的主題',
    'topics.new.desc'           => '最近添加到網站的新主題。 主題統計和訂閱主題的能力',
    
    'topics.my'                 => '我讀',
    'topics.my.info'            => '我關注的話題，我閱讀',
    'topics.my.desc'            => '我閱讀或創建的主題',
    
    'main.feed.title'           => '{name} — 社區（主題、感興趣的博客）',
    'main.top.title'            => '{name} — 熱門帖子',
    'main.all.title'            => '{name} — 所有帖子 ',
    'main.feed.desc'            => '興趣主題、提要、博客。 對話、問答、有趣的事實。 腳本 CMS 和社交網絡、框架。 腳本 {name}.',
    'main.top.desc'             => '社區提要中的熱門帖子列表（按回複數）。 興趣話題。 對話、問答、評論。 腳本 {name}.',
    'main.all.desc'             => '社區提要中所有帖子的列表。 腳本 {name}.',
    
    'main.feed.info'            => '按時間順序發帖...',
    'main.all.info'             => '社區提要中的所有帖子...',
    'main.top.info'             => '最近的活躍帖子...',
    'main.deleted.title'        => '已刪除的帖子',
    
    'comments.all.info'         => '所有評論...',
    'comments.deleted.info'     => '刪除評論...',
    'answers.all.info'          => '所有答案...',
    'answers.deleted.info'      => '已刪除的回复...',
    
    'comments.desc'     => '所有評論（關於答案）都在社區中按優先級排序',
    'answers.desc'      => '所有（對帖子的）回复都按社區中的優先級排序',
    'domain.desc'       => '按域對社區中的帖子進行分組',
    
    'tl0.title'         => '客人',
    'tl1.title'         => '用戶',
    'tl2.title'         => '會員',
    'tl3.title'         => '居民',
    'tl4.title'         => '常客',
    'tl5.title'         => '領袖',
    'tl10.title'        => '職員',
    
    /* 管理面板 */
    'submenu'           => '子菜單',
    'useful.resources'  => '有用的資源',
    'tools'             => '儀器',
    'logs'              => '日誌',
    'buttons'           => '鈕扣',
    'freely'            => '自由',
    'words'             => '停用詞',
    'word'              => '停用詞',
    'reward.weight'     => '獎勵權重',
    'update.data'       => '更新數據',
    'build'             => '收集',
    'test.email'        => '測試消息系統',
    'css'               => 'Css',
    
    /* 網站目錄 */
    'soft'                  => '柔軟的',
    'status'                => '鏈接狀態',
    'there.program'         => '有一個程序',
    'hosted.github'         => '它託管在 GitHub 上',
    'url.address.github'    => 'GitHub 存儲庫 URL',
    'by.date'               => '按日期',
    'directory.info'        => '按主题标题系统化的网站目录',
    'results.search'        => '網站：約。',
    'no.search.results'     => '搜索沒有返回任何結果。 <br>搜索查詢太短/太長',
    'empty.request'         => '空搜索詞',
    'web.bookmarks.info'    => '按网站划分的书签',
    'web.banner.info'       => '带有注释和屏幕截图的手动选择站点目录。 主题、区域和典型的摩擦. 能够通过描述搜索',

    'news.media'            => '新聞和媒體',
    'social.network'        => '社交網絡',
    'internet'              => '互聯網',
    'software'              => '程式',
    'reference.info'        => '參考信息',
    'reference.help'        => '百科全書、參考書',
    'science.education'     => '科學與教育',
    'goods.services'        => '貨物與服務',
    'private.life'          => '家',
    'private.life.help'     => '兒童，動物，愛好...',
    'security'              => '保安...',

    'web.top.view'          => '按评级划分',
    'web.view'              => '新网站',
    'web.audits.view'       => '需要考虑',
    'web.bookmarks.view'    => '我的书签',
    'web.deleted.view'      => '已删除',

    'web.home.title'        => '網站目錄',
    'web.home.desc'         => '目录中的站点列表按时间顺序排列。 能够搜索，书签，按受欢迎程度排序',
    'web.top.home'          => '按目录评级划分的网站',
    'web.top.home.title'    => '网站目录(按评级)',
    'web.top.home.desc'     => '按评级排序的网站。 资源的截图和描述，搜索和书签的能力',

    'web.top.cat.title'     => '{name} — 人氣網站',
    'web.top.cat.desc'      => '按主題公佈的網站列表 {name}. {desc}',
    'web.all.cat.title'     => '{name} — 量規，網站按日期',
    'web.all.cat.desc'      => '專欄中按時間順序排列的網站 {name}. {desc}',
    
    'web.audits.home.title' => '網站審核',
    'web.audits.home.desc'  => '正在考慮的網站',
    
    'site.add'              => '添加站點',
    'site.edit'             => '編輯網站',
    'web.user.sites.view'   => '我的網站',
    'no.user.sites'         => '還沒有網站',
    'no.bookmarks.sites'    => '還沒有書籤',
    'user.sites.info'       => '我添加的網站列表',
    'moderation'            => '適度',
    
    'deny.replies'          => '拒絕回复？',
    'discussions.closed'    => '禁止討論該網站。 作者或工作人員已結束討論。',
    
    'add.site.help'         => '<i class="gray-600">標題格式:</i><br> «LibArea» — 社區<br><br><i class="gray-600">說明（不含廣告）:</i><br> ...網站上有什麼?<i>',
    
    /* 團隊 */
    'no.teams'          => '不幸的是，還沒有命令',
    'team.created'      => '團隊創建',
    'team.change'       => '命令更改成功',
    'team.reestablish'  => '該命令已被刪除',
    'team.info'         => '您最多可以創建 5 個團隊一起工作',
    'no.users'          => '沒有團隊成員',
    
    /* 搜索 */
    'results.search'    => '結果：大約。',
    'no.search.results' => '搜索沒有返回任何結果。 <br>搜索查詢太短/太長',
    'empty.request'     => '空搜索詞',
    'search.home'       => '<h2>搜索</h2>輸入搜索詞組',
    'search.title'      => '搜索系統',
    'search.desc'       => '在網站和目錄上搜索，考慮到形態、方面。搜索系統 {name}',
    'search.help'       => '網站搜索等...',
    'schemas'           => '方案',
    'query'             => '要求',
    'rebuild.index'     => '重建索引',
    'end.window.close'  => '重建後窗口將關閉',
];

return $data;
