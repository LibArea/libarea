...
class Html
{
    ...
    public static function facets($facet, $type, $css, $sort = 'all')
    {
        ...
        foreach (array_chunk($facets, 3) as $row) :
            if ($row[0] == $type) {
                if ($type === 'category') {
                    $result[] = '<a class="' . $css . '" href="' . url($type, ['sort' => $sort, 'slug' => $row[1]]) . '">' . htmlEncode($row[2]) . '</a>';
                } else {
                    $result[] = '<a class="' . $css . '" href="' . url($type, ['slug' => $row[1]]) . '">' . htmlEncode($row[2]) . '</a>';
                }
            }
        endforeach;
    ...
    public static function facets_blog($blog_slug, $facet, $css)
    {
        ...
        foreach (array_chunk($facets, 3) as $row) :
            if ($row[0] === 'topic') {
                $result[] = '<a class="' . $css . '" href="' . url('blog.topic', ['slug' => $blog_slug, 'tslug' => $row[1]]) . '">' . htmlEncode($row[2]) . '</a>';
            }
        endforeach;
    ...
    public static function addPost($facet_id)
    {
		$html = '<div class="relative">
			<div class="trigger pointer">
			  ' . icon('icons', 'write', '24', 'icon large sky icon-bold') . '
			</div>
    ...
    public static function votes($content, $type, $icon = 'heart')
    {
        ...
        $html = '<div class="flex gap-sm gray-600"><div class="up-id click-no-auth">' . icon('icons', $icon) . '</div><div class="score"> ' . $count . '</div></div>';

        if (UserData::checkActiveUser()) {
            if (UserData::getUserId() == $content[$type . '_user_id']) {
                $html = '<div class="active flex gap-sm"><div class="up-id">' . icon('icons', $icon) . '</div><div class="score">' . $count . '</div></div>';
            } else {
                $active = $content['votes_' . $type . '_user_id'] ? 'active ' : '';
                $html = '<div id="up' . $content[$type . '_id'] . '" class="' . $active . 'flex gap-sm gray-600"><div data-id="' . $content[$type . '_id'] . '" data-type="' . $type . '" class="up-id pointer">' . icon('icons', $icon) . '</div><div class="score">' . $count . '</div></div>';
            }
        }
    ...
    public static function favorite($content_id, $type, $tid, $heading = '')
    {
        ...
        $html = '<div class="click-no-auth gray-600">' .  icon('icons', 'bookmark') . '' . $head . '</div>';
        if (UserData::checkActiveUser()) {
            $active = $tid ? 'active' : 'gray-600';
            $html = '<div id="favorite_' . $content_id . '" class="add-favorite pointer ' . $active . '" data-id="' . $content_id . '" data-type="' . $type . '">' .  icon('icons','bookmark') . '</i>' . $head . '</div>';
        }
