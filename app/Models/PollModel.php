...
class PollModel extends \Hleb\Scheme\App\Models\MainModel
{
    ...
    {
        $sql = "SELECT poll_id, poll_title, poll_user_id, poll_date FROM poll_questions WHERE poll_id = :question_id";
    ...
    {
        $sql = "SELECT SUM(answer_votes) as sum FROM poll_answers WHERE answer_question_id = :question_id";
    ...
    {
        $sql = "SELECT answer_id, answer_title, answer_votes FROM poll_answers WHERE answer_question_id = :question_id";
    ...
    {
        $sql = "SELECT answer_id FROM poll_answers WHERE answer_question_id = :question_id";
    ...
    {
        $start  = ($page - 1) * $limit;
        $sql = "SELECT poll_id, poll_title, poll_date FROM poll_questions WHERE poll_user_id = :user_id ORDER BY poll_id DESC LIMIT :start, :limit";
    ...
    {
        $sql = "SELECT poll_title, poll_date FROM poll_questions WHERE poll_user_id = :user_id";
    ...
    {
        DB::run("INSERT INTO poll_questions(poll_title, poll_user_id) VALUES(:title, :user_id)", ['title' => $title, 'user_id' => UserData::getUserId()]);
    ...
    {
        if (is_int($key)) {
            DB::run("INSERT INTO poll_answers(answer_question_id, answer_title) VALUES(:question_id, :title)", ['question_id' => $id, 'title' => $val]);
        }
    ...
    {
        $sql = "SELECT vote_id FROM poll_votes WHERE vote_question_id = :question_id AND vote_user_id = :user_id";
    ...
    public static function vote($question_id, $answer_id)
    {
        ...
        $sql = "UPDATE poll_answers SET answer_votes = answer_votes + 1 WHERE answer_id = :id AND answer_question_id = :question_id";

        DB::run($sql, ['question_id' => $question_id, 'id' => $answer_id]);

        $sql = "INSERT INTO poll_votes(vote_question_id, vote_user_id, vote_ip) VALUES(:question_id, :user_id, :ip)";
    ...
    {
        $sql = "UPDATE poll_questions SET poll_title = :title WHERE poll_id = :question_id";
    ...
    {
        $sql = "UPDATE poll_answers SET answer_title = :title WHERE answer_question_id = :question_id AND answer_id = :id";
