...
class UserModel extends \MainModel
{
    ...
    public static function contentCount($user_id, $type)
    {
        if ($type == 'posts') {
            ...
            $sql = "SELECT comment_id, comment_user_id, comment_is_deleted 
                    FROM comments WHERE comment_user_id = :user_id and comment_is_deleted = 0";
        } else {
            $sql = "SELECT answer_id, answer_user_id, answer_is_deleted 
                    FROM answers WHERE answer_user_id = :user_id and answer_is_deleted = 0";
        }
